...
<div class="w-full max-w-5xl pl-4 pr-4 sm:pl-6 sm:pr-6 lg:pl-8">
	...
	{#if view === "grid"}
		<div class="flex items-center justify-between mb-6">
			<h1 class="hidden sm:block text-lg font-semibold text-zinc-100">My Projects</h1>
			...
			<Dialog.Root bind:open={dialogOpen}>
				<Dialog.Trigger>
					{#snippet child({ props })}
						<Button
							{...props}
							class="bg-violet-600 hover:bg-violet-400 text-white hover:text-white gap-1.5 ml-auto"
							variant="ghost"
						>
							<Plus class="h-4 w-4" />
							Add Project
